...
import { Text, View, FlatList, TextInput, TouchableOpacity, KeyboardAvoidingView, Image } from 'react-native';
...
class Chat extends React.Component {
    ...
    render () {
        ...
        return (
            <KeyboardAvoidingView enable behavior='padding' style= {styles.container}>
                <FlatList
                    inverted
                    ...
                    renderItem={ ({item}) => (
                        <TouchableOpacity onPress={() => this.goToChat(item)} style={[styles.row, styles.space]}>
                            ...
                            <View style={[styles.container, item.uid === uid ? styles.right : styles.left]}>
                                ...
                                <Text style= {styles.grey}>{item.message}</Text>
                                <Text style= {[styles.grey, styles.small]}>{moment(item.date).format('ll')}</Text>
                            </View>
                            { item.uid === uid ? <Image style={styles.roundImage} source={{uri: item.photo}}/> : null}
                        </TouchableOpacity>
                    )
